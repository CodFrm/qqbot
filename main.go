...
func reconnect() *gosocketio.Client {
	...
	if err := c.On("OnFriendMsgs", func(h *gosocketio.Channel, args iotqq.Message) {
		if _, ok := config.AppConfig.FeatureMap["alimama"]; ok {
			if _, ok := args.CommandMatch("(.\\w{10,}.|http(s|):)"); len(args.CurrentPacket.Data.Content) > 6 && ((ok && args.CurrentPacket.Data.Content[:3] != "淘") || args.CurrentPacket.Data.Content[:4] == "转 ") {
